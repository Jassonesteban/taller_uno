...
public class ProjectController extends BaseController {
    ...
    public ProjectDTO saveProject(@Valid @RequestBody ProjectDTO projectDTO) throws Exception {
        Project response = this.projectService
                .saveProject((Project) new ModelMapper().map(projectDTO, Project.class));

        ProjectDTO responseDTO = new ModelMapper().map(response, ProjectDTO.class);
    ...
    public List<ProjectDTO> getProject() throws Exception {
        ...
        if (projects != null) {
            response = projects.stream()
                    .map(p -> (ProjectDTO) new ModelMapper().map(p, ProjectDTO.class))
